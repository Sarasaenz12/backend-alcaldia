...
def create_admin_user():
    """
    Función para crear automáticamente el usuario admin
    """
    try:
        import django
        from django.conf import settings
        
        # Esperar un poco para asegurar que Django esté completamente cargado
        import time
        time.sleep(2)
        ...
        if not CustomUser.objects.filter(email='admin@alcaldiacordoba.gov.co').exists():
            admin_user = CustomUser.objects.create_user(
                username='admin_alcaldia',
            ...
            print("✅ Usuario admin creado automáticamente:", admin_user.email)
        else:
            print("ℹ️ Usuario admin ya existe")
            
    except Exception as e:
        print(f"⚠️ Error creando admin automáticamente: {e}")

# Solo ejecutar en producción y si no estamos en migraciones
if not DEBUG and 'runserver' not in sys.argv and 'migrate' not in sys.argv:
    import sys
    # Usar threading para no bloquear el inicio de la aplicación
    threading.Timer(3.0, create_admin_user).start()
